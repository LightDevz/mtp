...
if [[ $distro =~ "Ubuntu" ]]; then
	apt update -y
	apt-get -y install python3 python3-pip sed git curl jq ca-certificates -y
elif [[ $distro =~ "Debian" ]]; then
	apt-get update -y
	apt-get install -y jq ca-certificates iptables-persistent iptables git sed curl wget python3 python3-pip -y
	#Firewall
	iptables -A INPUT -p tcp --dport "$PORT" --jump ACCEPT
	iptables-save >/etc/iptables/rules.v4
else
